...
import { allcustomers,newcustomer,inertmanycustomer,updatecustomer,deletecustomer} from '../../services/apicustomers/apicustomers';
const Customers = () =>{
 ...
 const getdata=async()=>{
    setLoading(true);
    var res=await allcustomers({first,rows,globalFilter});
  ...
  const save=async(event)=>{
     event.preventDefault();
      var res=await newcustomer(formdata);
 ...
 const importfile=async(event)=>{
  ...
  var res=await inertmanycustomer(selectedFile);
 ...
 const update=async(event)=>{
  event.preventDefault();
  var res=await updatecustomer(formdata);
  if(res.message=='success'){
 ...
 const accept = async(data) => {
  var res=await deletecustomer({id:data.id});
  if(res.message=='success'){
...
const handleClearFilter=(event)=>{
  ...
  <div className="Customers" data-testid="Customers">
    <AddForm showModal={showModal.modelname=="newform" || showModal.modelname=="editform"?showModal:false} handleCloseModal={handleCloseModal} formdata={formdata} changeHandler={changeHandler} save={save} update={update}  />
    ...
    <ToastContainer/>
    <ConfirmDialog />
    <Customerlist dt={dt} tabebodydata={tabebodydata} 
